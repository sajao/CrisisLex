...
if __name__ == "__main__": 
    ...
    pl = PrintListener()
    if len(c.output_filename) == 0:
        pl.set_output(open("your_json_file.json","w"))
    else:
        pl.set_output(open(c.output_filename,"w"))
        
    if len(c.input_filename) == 0:
        # recommended CrisisLex terms
        ...
        to_track = get_terms(c.input_filename)
    
    if len(c.additional_terms_file) > 0:
        additional_terms = get_terms(c.additional_terms_file)
        to_track = (additional_terms.extend(to_track))[:400]
